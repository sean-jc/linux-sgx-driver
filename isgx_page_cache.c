 ...
 * since we checked num_free.  Re-check after acquiring the semaphore
 * for writing, which locks out all readers from the entire list.
 */
void isgx_free_va_page(struct isgx_va_page *va_page)
{
	down_write(&isgx_va_pages_sem);
	if (likely(atomic_read(&va_page->used) == 0)) {
